          ...
          setup() {
              ...
              const _fetchData = async (endpoint, options = {}) => {
                  ...
                  try {
                      ...
                      const response = await fetch(`${apiBaseUrl.value}${endpoint}`, options); // Menggunakan apiBaseUrl.value
                      if (!response.ok) {
              ...
              const startDSSCalculation = async () => {
                  ...
                  } catch (error) { /* Error sudah ditangani oleh _fetchData */ }
              };

              const fetchExistingRankings = async () => {
                  console.log('Mengambil peringkat VIKOR yang ada...');
                  try {
                      const data = await _fetchData('vikor/rankings', { clearResultsOnFetch: dssResults.value.length > 0 ? false: undefined });
              ...
              const loadUserData = () => {
                  ...
                  if (storedUserData) {
                      ...
                      catch (e) { console.error('Data pengguna rusak di storage:', e); logout(); }
                  } else { console.warn('Tidak ada data pengguna. Menggunakan data default/sampel.'); }
              };
