...
createApp({
    setup() {
        ...
        // States untuk PDF download
        const isDownloadingPDF = ref(false);
        const rankingData = computed(() => dssResults.value);
        ...
        const showAlert = (message, type = 'info') => {
            // Implementasi alert sederhana - bisa diganti dengan library toast
            alert(message);
        };
        ...
        const checkAuth = () => {
            ...
            if (!storedUserData) {
                window.location.href = '/yayasan_bambu_dss/template/user/user_login_register.html';
                return false;
            }
            return true;
        };
        ...
        const downloadPDF = () => {
            ...
            html2pdf().from(element).set(options).save().then(() => {
                ...
                showAlert('Gagal membuat PDF. Silakan coba lagi.', 'error');
                isDownloadingPDF.value = false;
            });
        };
        ...
        const renderOrUpdateChart = () => {
            ...
            const labels = sortedResults.map(item =>
            ...
            chartInstance.value = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Q-Value',
                        data: dataValues,
                        backgroundColor: sortedResults.map((item, index) =>
                            index === 0 ? 'rgba(22, 163, 74, 0.85)' : 'rgba(74, 222, 128, 0.65)'
                        ),
                        borderColor: sortedResults.map((item, index) =>
                            index === 0 ? 'rgba(17, 128, 59, 1)' : 'rgba(50, 180, 100, 1)'
                        ),
                        borderWidth: 1,
                        barThickness: 'flex',
                        maxBarThickness: 30,
                ...
                options: {
                    ...
                    scales: {
                        x: {
                            ...
                            title: {
                                display: true,
                                text: 'Q-Value (Semakin Rendah Semakin Baik)',
        ...
        const fetchExistingRankings = async () => {
            ...
            try {
                ...
                dssResults.value = (data || []).map(r => ({
                    ...
                    S: (r.S !== undefined && !isNaN(parseFloat(r.S)) ? parseFloat(r.S) :
                        (r.score !== undefined && !isNaN(parseFloat(r.score)) ? parseFloat(r.score) : NaN)).toFixed(6),
                    R: (r.R !== undefined && !isNaN(parseFloat(r.R)) ? parseFloat(r.R) : NaN).toFixed(6),
                    Q: (r.Q !== undefined && !isNaN(parseFloat(r.Q)) ? parseFloat(r.Q) :
                        (r.score !== undefined && !isNaN(parseFloat(r.score)) ? parseFloat(r.score) : NaN)).toFixed(6),
                })).map(item => ({
                    ...item,
        ...
        watch(dssResults, (newVal, oldVal) => {
            if (newVal && newVal.length > 0 &&
                (JSON.stringify(newVal) !== JSON.stringify(oldVal) || !chartInstance.value)) {
                nextTick(() => {
                    renderOrUpdateChart();
                });
